<?php
// app/Views/_partials/post_card.php
// Expects $post array and $isLoggedIn boolean to be available
...
?>
<article class="bg-card border rounded-lg overflow-hidden flex flex-col" data-post-container-id="<?php echo $post['post_id']; ?>">
         ...
         <div class="flex-grow">
          ...
          <?php if ($isAuthor): ?>
              <div class="relative post-options-dropdown">
                 <button type="button" aria-label="Post options" class="cursor-pointer transition post-options-button p-1 rounded-full text-muted-foreground hover:bg-accent hover:text-foreground focus:outline-none focus:ring-1 focus:ring-ring">
                 ...
                 <div class="post-options-menu hidden absolute right-0 mt-1 w-36 bg-popover border rounded-md shadow-lg z-10 py-1 text-sm">
                    <button type="button" class="cursor-pointer transition post-edit-button block w-full text-left px-3 py-1.5 text-foreground hover:bg-accent">Edit Post</button>
                    <button type="button" class="cursor-pointer transition post-delete-button block w-full text-left px-3 py-1.5 text-destructive hover:bg-destructive/10">Delete Post</button>
                 </div>
              </div>
          <?php endif; ?>
     </div>
    ...
    <div class="post-content-area px-4 <?php echo !empty($post['content']) || !empty($post['image_url']) ? 'pb-4' : 'pb-1'; ?>">
            ...
            <form class="post-edit-form hidden mt-2" data-post-id="<?php echo $post['post_id']; ?>">
             <?php echo csrf_field(); ?>
                 ...
                 <div class="flex justify-end items-center space-x-2 mt-3">
                      ...
                      <button type="button" class="cursor-pointer edit-cancel-button inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-8 px-3">Cancel</button>
                     <button type="submit" class="cursor-pointer edit-save-button inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 h-8 px-3">Save Changes</button>
                 </div>
            </form>
        <?php endif; ?>
    </div>
     ...
     <div class="p-2 border-t grid grid-cols-2 gap-1">
         <button data-post-id="<?php echo $post['post_id']; ?>" aria-label="<?php echo $post['user_liked'] ? 'Unlike' : 'Like'; ?> post" aria-pressed="<?php echo $post['user_liked'] ? 'true' : 'false'; ?>" class="cursor-pointer like-button flex items-center justify-center space-x-1.5 py-1.5 px-3 rounded-md hover:bg-accent transition-colors <?php echo $post['user_liked'] ? 'text-red-500 font-medium' : 'text-muted-foreground'; ?> <?php echo !$isLoggedIn ? 'cursor-not-allowed opacity-60' : ''; ?>" <?php echo !$isLoggedIn ? 'disabled title="Login to like posts"' : ''; ?>>
         ...
         <button data-post-id="<?php echo $post['post_id']; ?>" aria-expanded="false" aria-controls="comment-section-<?php echo $post['post_id']; ?>" class="cursor-pointer comment-toggle-button flex items-center justify-center space-x-1.5 py-1.5 px-3 rounded-md text-muted-foreground hover:bg-accent transition-colors <?php echo !$isLoggedIn ? 'cursor-not-allowed opacity-60' : ''; ?>" <?php echo !$isLoggedIn ? 'disabled title="Login to comment"' : ''; ?>>
     ...
     <div id="comment-section-<?php echo $post['post_id']; ?>" class="comment-section border-t px-4 py-3 space-y-3 hidden">
        <div class="comment-list space-y-3 text-sm">
         <?php // --- Replaced text with spinner --- ?>
